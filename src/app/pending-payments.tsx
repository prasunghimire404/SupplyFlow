...
import { get as dbGet, ref as dbRef, remove as dbRemove } from 'firebase/database';
import { AlertCircle, ArrowLeft, Calendar, ChevronRight, Clock, Plus, Search } from 'lucide-react-native';
...
export default function PendingPaymentsScreen() {
    ...
    const [activeFilter, setActiveFilter] = useState<'All' | 'Overdue' | 'Upcoming'>('All');
    ...
    const loadSuppliers = async () => {
        ...
        try {
            const snap = await dbGet(dbRef(rtdb, `workspaces/${user.activeWorkspaceId}/suppliers`));
            if (snap.exists()) {
                const data = snap.val();
                const list: Supplier[] = Object.entries(data).map(([id, val]: [string, any]) => ({
                    id,
    ...
    const filterData = () => {
        ...
        if (activeFilter === 'Overdue') {
            ...
            result = result.filter(s => s.status === 'upcoming' || s.status === 'due_today');
        }
    ...
    const markAsPaid = async (supplierId: string) => {
        if (!user?.activeWorkspaceId) return;
        try {
            await dbRemove(dbRef(rtdb, `workspaces/${user.activeWorkspaceId}/suppliers/${supplierId}`));
    ...
    const overdueAmount = suppliers.filter(s => s.status === 'overdue').reduce((sum, s) => sum + s.amount, 0);
    const overdueCount = suppliers.filter(s => s.status === 'overdue').length;

    const getStatusConfig = (supplier: Supplier) => {
        const now = Date.now();
        ...
        <View className="flex-1 bg-background">
            ...
            <View className="flex-row px-4 mt-2 mb-4">
                <View className="bg-surface rounded-2xl p-3 border border-border flex-1 mx-1 items-center">
                    ...
                    <Text className="text-white font-bold mt-1" style={{ fontFamily: 'DMSans_700Bold' }}>
                        Rs {totalDue.toLocaleString('en-IN')}
                    </Text>
                </View>
                <View className={`bg-surface rounded-2xl p-3 border flex-1 mx-1 items-center ${overdueCount > 0 ? 'border-accent-violet' : 'border-border'}`}>
                    <Text className="text-text-muted text-xs" style={{ fontFamily: 'DMSans_400Regular' }}>
                        Overdue
                    </Text>
                    <Text className={`font-bold mt-1 ${overdueCount > 0 ? 'text-accent-violet' : 'text-white'}`} style={{ fontFamily: 'DMSans_700Bold' }}>
            ...
            <View className="flex-row px-4 mb-3">
                {(['All', 'Overdue', 'Upcoming'] as const).map((filter) => (
                    <Pressable
                        ...
                        className={`rounded-full px-4 py-2 mr-2 ${activeFilter === filter ? 'bg-primary' : 'bg-surface-light border border-border'
                            }`}
                    >
                        <Text
            ...
            <ScrollView className="flex-1 px-4">
                {suppliers.length === 0 ? (
                    <View className="items-center justify-center py-20">
                        ...
                        <Text className="text-text-secondary mt-4" style={{ fontFamily: 'DMSans_500Medium' }}>
                            No pending payments
                        </Text>
                        <Text className="text-text-muted text-sm mt-1 text-center px-8" style={{ fontFamily: 'DMSans_400Regular' }}>
                        ...
                        >
                            ...
                            <Text className="text-white font-medium ml-2" style={{ fontFamily: 'DMSans_500Medium' }}>
                                Add Supplier
                            </Text>
                        </Pressable>
                    </View>
                        ...
                        return (
                            ...
                            >
                                ...
                                <View className="items-end mr-2">
                                    ...
                                    <Text className={`text-xs mt-1 ${config.textColor}`} style={{ fontFamily: 'DMSans_500Medium' }}>
                                        {config.text}
                                    </Text>
                                </View>
